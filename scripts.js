...
function createEventCard(event) {
    ...
    // Map event types to colors for the tag
    const typeColors = {
        'Keynote': 'bg-purple-600 dark:bg-purple-800',
        'Workshop': 'bg-blue-600 dark:bg-blue-800',
        'Talk': 'bg-cyan-600 dark:bg-cyan-800',
        'Panel': 'bg-red-600 dark:bg-red-800',
        'Social': 'bg-green-700 dark:bg-green-900',
    };
    const colorClass = typeColors[event.type] || 'bg-gray-500 dark:bg-gray-700';
    ...
    return `
        <article class="event-card rounded-xl shadow-lg hover:shadow-2xl transition duration-300 overflow-hidden relative border border-gray-200 dark:border-gray-700 transform hover:-translate-y-1" style="background-color: var(--bg-secondary); color: var(--text-primary);">
            ...
            <div class="absolute top-3 right-3 ${event.color} px-4 py-1 text-white text-xs font-bold rounded-full shadow-lg z-10 uppercase tracking-wider">
                ${event.track}
            <div class="absolute top-3 right-3 ${colorClass} px-4 py-1 text-white text-xs font-bold rounded-full shadow-lg z-10 uppercase tracking-wider">
                ${event.type}
            </div>
            
            <div class="p-6">
                <p class="text-sm font-semibold mb-2" style="color: var(--color-accent);">${event.time}</p>
                <p class="text-sm font-semibold mb-2" style="color: var(--color-accent);">${time}</p>
                ...
                <button onclick="showDetails('${event.title}')" 
...
function showDetails(eventTitle) {
    ...
    messageBox.innerHTML = `
        <div class="bg-secondary p-8 rounded-xl shadow-2xl max-w-lg w-full transform transition-transform duration-300 scale-95 border-t-8 border-accent-cyan" style="background-color: var(--bg-secondary);">
            ...
            <p class="text-lg font-medium mb-2" style="color: var(--text-secondary);">${time}</p>
            <div class="${colorClass} inline-block px-3 py-1 rounded-full text-white font-semibold text-sm mb-6">${event.type}</div>
            <p class="leading-relaxed" style="color: var(--text-primary);">${event.description}</p>
